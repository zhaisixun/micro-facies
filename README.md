...
## Well-log facies baseline (data-level sliding window)
You can train ConvNeXt on an xlsx well-log file where each sheet is a well and
contains columns `GR`, `GR_diff1`, `GR_diff2`, and `facies`.
...
python main.py \
  ...
  --train_wells EP10-2-1,EP21-3-1,HZ26-6-1,LF8-1-3 \
  --val_wells KP11-4-3,LF8-1-1 \
  --feature_cols GR,GR_diff1,GR_diff2 \
  --label_col facies \
  --window_size 128 \
  --model convnext_tiny \
  --input_size 224 \
