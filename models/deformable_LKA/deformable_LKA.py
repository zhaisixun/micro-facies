...
class deformable_LKA(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = DeformConv(dim, groups=dim, kernel_size=(5, 5), padding=2)
        self.conv_spatial = DeformConv(
            dim, groups=dim, kernel_size=(7, 7), stride=1, padding=9, dilation=3)
...
class deformable_LKA_experimental(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = DeformConv_experimental(dim, groups=dim, kernel_size=(5, 5), padding=2)
...
class deformable_LKA_Attention(nn.Module):
    def __init__(self, d_model):
        ...
        self.spatial_gating_unit = deformable_LKA(d_model)   # deformabel LKA 是将传统LKA模块的spatial attention替换为deformable convolution
