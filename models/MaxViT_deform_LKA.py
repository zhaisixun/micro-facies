...
# 保证从任意 cwd 运行本文件时能找到 networks、deformable_LKA 等包（均以 2D/ 为根）
_2D_ROOT = Path(__file__).resolve().parent.parent
if str(_2D_ROOT) not in sys.path:
    sys.path.insert(0, str(_2D_ROOT))
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, C)
        x = rearrange(x, "b h w (p1 p2 c)-> b (h p1) (w p2) c", p1=2, p2=2, c=C // 4)       # 长宽×2，通道数÷4
        x = x.view(B, -1, C // 4)
        x = self.norm(x.clone())
...
class MaxViT_deformableLKAFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Encoder
        self.backbone = MaxViT4Out_Small(n_class=num_classes, img_size=224)    # 可换convnext
    ...
    def forward(self, x):
        ...
        output_enc_3, output_enc_2, output_enc_1, output_enc_0 = self.backbone(x)   # encoder
        # output_enc_3: [2, 768, 7, 7]   最底层maxvit block
        # output_enc_2: [2, 384, 14, 14]
        # output_enc_1: [2, 192, 28, 28]
        # output_enc_0: [2, 96, 56, 56]  最顶层maxvit block
